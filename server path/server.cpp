...
void transact_with_client(char * backup_dir, int sock)
{
	...
	while(n!=0)
	{
		...
		if (strcmp(ch,"filename")==0)
		{
			...
			bzero(fn,sizeof(fn));
			n=read(sock,fn,sizeof(fn));         		    	//  read filename
			...
			string makedir=path+ch;                         //Create directory 
			
			mkdir(makedir.c_str(),0777);
			st=path+ch+"/"+st;    					
			cout<<"Path of file to be backed up is : "<<st<<endl;			
			bzero(direc,sizeof(direc));
		...
		n =read(sock,ch,sizeof(ch));                             // Reading content of file1,   file2,  ....
	}
}
